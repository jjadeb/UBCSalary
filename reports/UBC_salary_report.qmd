...
```
 ...
 The accuracy on the test set was `{python} accuracy`. However, the accuracy on our UBC staff data is very likely lower than `{python} accuracy` since the UBC staff names that still need predictions are unique and unusual enough that were not found in our babyname corpus. 
